...
struct jc
{
    string protocol_stack =
        ...
        "1\",\"clt_ip\":\"\",\"port\":2404},{\"srv_ip\":\"127.0.0.1\","
        "\"clt_"
        "ip\":"
...
/*TEST(IEC104, PluginGetJSonConfig)
{
    IEC104 iec;
    struct jc config;
    iec.setJsonConfig(config.protocol_stack, config.exchanged_data,
                      config.protocol_translation, config.tls);

    json stack_config =
        json::parse(config.protocol_stack)["default"]["protocol_stack"];

    ASSERT_FALSE(iec.getConfigValue<string>(stack_config, "/name"_json_pointer)
                     .length() == 0);

    string expectedResult("iec104client");
    ASSERT_EQ(iec.getConfigValue<string>(stack_config, "/name"_json_pointer),
              expectedResult);
}*/
