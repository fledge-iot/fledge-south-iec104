                           ...
                           const std::string& tls_configuration)
{
    ...
    try
        {
            m_msg_configuration =
                json::parse(msg_configuration)["exchanged_data"];
        }
    catch (json::parse_error& e)
        {
            Logger::getLogger()->fatal(
    ...
    try
        ...
        {
            Logger::getLogger()->fatal(
                "Couldn't read tls_conf json config string : " +
                string(e.what()));
        }
}
                ...
                for (int i = 0; i < CS101_ASDU_getNumberOfElements(asdu); i++)
                    {
                        ...
                        if (!(label = IEC104::m_checkExchangedDataLayer(
                                  ca, "M_ME_NB_1", ioa))
                                 .empty())
                            {
                                auto io_casted = (MeasuredValueScaled)io;
                                long int value = MeasuredValueScaled_getValue(
                                    (MeasuredValueScaled)io_casted);
                                QualityDescriptor qd =
                                    MeasuredValueScaled_getQuality(io_casted);
                                mclient->addData(datapoints, ioa, label, value,
                                                 qd);
                                ...
                                if (m_comm_wttag)
                                    {
                                        CP56Time2a ts =
                                            SinglePointWithCP56Time2a_getTimestamp(
                                                io_casted);
                                        bool is_invalid =
                                            CP56Time2a_isInvalid(ts);
                                        if (m_tsiv == "PROCESS" || !is_invalid)
                                            mclient->addData(datapoints, ioa,
                                                             label, value, qd,
                                                             ts);
                                    }
                                else
                                    mclient->addData(datapoints, ioa, label,
                                                     value, qd);
                ...
                for (int i = 0; i < CS101_ASDU_getNumberOfElements(asdu); i++)
                    {
                        InformationObject io = CS101_ASDU_getElement(asdu, i);
                        long ioa = InformationObject_getObjectAddress(io);
                        label = IEC104::m_checkExchangedDataLayer(
                            ca, "M_ME_TF_1", ioa);
                        if (!label.empty())
                            {
                                auto io_casted =
                               ...
                               "/transport_layer/conn_passv"_json_pointer))
        {
            ...
            if (m_getConfigValue<bool>(
                    ...
                    "/application_layer/time_sync"_json_pointer))
                {
                    ...
                    CP56Time2a_createFromMsTimestamp(&currentTime,
                                                     Hal_getTimeInMs());
...
void IEC104::start()
{
    ...
    for (auto& path_element :
         m_stack_configuration["transport_layer"]["connection"]["path"])
        {
            ...
            try
                {
                    string ip = m_getConfigValue<string>(
                        path_element, "/srv_ip"_json_pointer);
                    int port = m_getConfigValue<int>(path_element,
                                                     "/port"_json_pointer);
                    ...
                    if (m_getConfigValue<bool>(
                        ...
                        new_connection =
                            m_createTlsConnection(ip.c_str(), port);
                    else
                        new_connection =
                            CS104_Connection_create(ip.c_str(), port);
                    Logger::getLogger()->info("Connection created");
                }
            catch (exception& e)
                {
                    Logger::getLogger()->error(
                        "Exception while creating connection", e.what());
                    throw e;
                }
            ...
            CS104_Connection_setASDUReceivedHandler(
                new_connection, m_asduReceivedHandler,
...
T IEC104::m_getConfigValue(json configuration, json_pointer<json> path)
{
    ...
    try
        ...
        {
            Logger::getLogger()->fatal("Couldn't parse value " +
                                       path.to_string() + " : " + e.what());
        }
    catch (json::out_of_range& e)
        {
            Logger::getLogger()->fatal("Couldn't reach value " +
                                       path.to_string() + " : " + e.what());
        }
            ...
            for (auto connection : m_connections)
                {
                    ...
                    if (CS104_Connection_sendInterrogationCommand(
                            connection, CS101_COT_ACTIVATION, ca,
                                              ...
                                              unsigned int ioa)
{
    ...
    for (auto& element : m_msg_configuration["asdu_list"])
        {
            if (m_getConfigValue<unsigned int>(element, "/ca"_json_pointer) ==
                ca)
                {
                    known_ca = true;
                    if (m_getConfigValue<string>(
                            element, "/type_id"_json_pointer) == type_id)
                        {
                            known_type_id = true;
                            if (m_getConfigValue<unsigned int>(
                                    element, "/ioa"_json_pointer) == ioa)
            ...
            if (*flag)  // Operation has been validated
                {
                    Logger::getLogger()->info(
                        id + " completed in " +
                        to_string(time_span.count() / 1000) + " seconds.");
                    return 0;
                }
                    ...
                    if (time_span.count() > 1000 * delay)  // Time has expired
                        {
                            Logger::getLogger()->warn(
                                id + " is taking to long, restarting ...");
                            return 1;
                        }
                }
        }
         ...
         (*m_pivot_configuration)["mapping"]["data_object_header"].items())
        {
            if (feature.value() == "type_id")
                data_header->push_back(m_createDatapoint(
                    feature.key(), (long)CS101_ASDU_getTypeID(asdu)));
            else if (feature.value() == "ca")
                data_header->push_back(m_createDatapoint(
                    feature.key(), (long)CS101_ASDU_getCA(asdu)));
            else if (feature.value() == "oa")
                data_header->push_back(m_createDatapoint(
                    feature.key(), (long)CS101_ASDU_getOA(asdu)));
            else if (feature.value() == "cot")
                data_header->push_back(m_createDatapoint(
                    feature.key(), (long)CS101_ASDU_getCOT(asdu)));
            else if (feature.value() == "istest")
                data_header->push_back(m_createDatapoint(
                    feature.key(), (long)CS101_ASDU_isTest(asdu)));
            else if (feature.value() == "isnegative")
                data_header->push_back(m_createDatapoint(
                    feature.key(), (long)CS101_ASDU_isNegative(asdu)));
        }
    ...
    for (auto connection : m_connections)
        {
            if (operation.compare(
                    "CS104_Connection_sendInterrogationCommand") == 0)
                {
                    int casdu = atoi(params[0]->value.c_str());
                    CS104_Connection_sendInterrogationCommand(
                        connection, CS101_COT_ACTIVATION, casdu,
                        IEC60870_QOI_STATION);
                         ...
                         "CS104_Connection_sendTestCommandWithTimestamp") == 0)
                {
                    ...
                    CP56Time2a_createFromMsTimestamp(&testTimestamp,
                                                     Hal_getTimeInMs());
                    CS104_Connection_sendTestCommandWithTimestamp(
            ...
            else if (operation.compare("SingleCommandWithCP56Time2a") == 0)
                {
                    ...
                    bool value =
                        static_cast<bool>(atoi(params[2]->value.c_str()));
                    struct sCP56Time2a testTimestamp;
                    CP56Time2a_createFromMsTimestamp(&testTimestamp,
                                                     Hal_getTimeInMs());
                    ...
                    Logger::getLogger()->info(
                        "SingleCommandWithCP56Time2a send");
            ...
            else if (operation.compare("DoubleCommandWithCP56Time2a") == 0)
                {
                    ...
                    Logger::getLogger()->info(
                        "DoubleCommandWithCP56Time2a send");
