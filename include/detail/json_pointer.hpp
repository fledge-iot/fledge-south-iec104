...
public:
    ...
    /// @brief append an unescaped reference token at the end of this JSON
    /// pointer
    /// @sa https://json.nlohmann.me/api/json_pointer/operator_slasheq/
    ...
    friend json_pointer operator/(
        ...
        std::string token)  // NOLINT(performance-unnecessary-value-param)
    {
        return json_pointer(lhs) /= std::move(token);
    }

    /// @brief create a new JSON pointer by appending the array-index-token at
    /// the end of the JSON pointer
    /// @sa https://json.nlohmann.me/api/json_pointer/operator_slash/
    friend json_pointer operator/(const json_pointer& lhs,
    ...
    void pop_back()
    {
        if (JSON_HEDLEY_UNLIKELY(empty()))
        {
            JSON_THROW(detail::out_of_range::create(
                405, "JSON pointer has no parent", BasicJsonType()));
        }
    ...
    static typename BasicJsonType::size_type array_index(const std::string& s)
    {
        ...
        if (JSON_HEDLEY_UNLIKELY(s.size() > 1 && s[0] == '0'))
        {
            JSON_THROW(detail::parse_error::create(
                106, 0, "array index '" + s + "' must not begin with '0'",
                BasicJsonType()));
        }
        ...
        {
            JSON_THROW(detail::parse_error::create(
                109, 0, "array index '" + s + "' is not a number",
                BasicJsonType()));
        }
        ...
        {
            JSON_THROW(detail::out_of_range::create(
                410, "array index " + s + " exceeds size_type",
                BasicJsonType()));  // LCOV_EXCL_LINE
        }
                ...
                */
                ...
                default:
                    JSON_THROW(detail::type_error::create(
                        313, "invalid value to unflatten", j));
            }
        }
