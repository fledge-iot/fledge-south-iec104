...
void from_json(const BasicJsonType& j, typename std::nullptr_t& n)
{
    if (JSON_HEDLEY_UNLIKELY(!j.is_null()))
    {
        JSON_THROW(type_error::create(
            302, "type must be null, but is " + std::string(j.type_name()), j));
    }
    n = nullptr;
}
...
void from_json(const BasicJsonType& j, typename BasicJsonType::binary_t& bin)
{
    ...
    {
        JSON_THROW(type_error::create(
            302, "type must be binary, but is " + std::string(j.type_name()),
            j));
    }
...
void from_json_tuple_impl(BasicJsonType&& j, std::pair<A1, A2>& p,
                          priority_tag<1> /*unused*/)
{
...
template <typename BasicJsonType, typename... Args>
void from_json_tuple_impl(BasicJsonType&& j, std::tuple<Args...>& t,
                          priority_tag<3> /*unused*/)
{
               ...
               std::map<Key, Value, Compare, Allocator>& m)
{
    ...
    for (const auto& p : j)
    {
        ...
        {
            JSON_THROW(type_error::create(
                302, "type must be array, but is " + std::string(p.type_name()),
                j));
        }
...
{
constexpr const auto& from_json = detail::static_const<
