...
protected:
    ...
    static std::string diagnostics(const BasicJsonType& leaf_element)
    {
        ...
        for (const auto* current = &leaf_element; current->m_parent != nullptr;
             current = current->m_parent)
        {
            switch (current->m_parent->type())
            {
                ...
                {
                    for (std::size_t i = 0;
                         i < current->m_parent->m_value.array->size(); ++i)
                    {
                        if (&current->m_parent->m_value.array->operator[](i) ==
                            current)
                        {
                ...
                case value_t::object:
                {
                    for (const auto& element :
                         *current->m_parent->m_value.object)
                    {
        ...
        return "(" +
               std::accumulate(tokens.rbegin(), tokens.rend(), std::string{},
...
public:
    ...
    static invalid_iterator create(int id_, const std::string& what_arg,
                                   const BasicJsonType& context)
    {
        std::string w = exception::name("invalid_iterator", id_) +
                        exception::diagnostics(context) + what_arg;
        return {id_, w.c_str()};
    }
...
public:
    ...
    static out_of_range create(int id_, const std::string& what_arg,
                               const BasicJsonType& context)
    {
        std::string w = exception::name("out_of_range", id_) +
                        exception::diagnostics(context) + what_arg;
        return {id_, w.c_str()};
    }
...
public:
    ...
    static other_error create(int id_, const std::string& what_arg,
                              const BasicJsonType& context)
    {
        std::string w = exception::name("other_error", id_) +
                        exception::diagnostics(context) + what_arg;
        return {id_, w.c_str()};
    }
