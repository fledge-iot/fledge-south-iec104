        ...
        else
        {
            ...
            else
            {
                if (JSON_HEDLEY_UNLIKELY(!input.empty()))
                {
                    ...
                    const auto charcode =
                        0x10000u +
                        (((static_cast<unsigned int>(wc) & 0x3FFu) << 10u) |
                         (wc2 & 0x3FFu));
                    utf8_bytes[0] =
    ...
    IteratorType, enable_if_t<is_iterator_of_multibyte<IteratorType>::value>>
{
    ...
    using adapter_type =
        wide_string_input_adapter<base_adapter_type, char_type>;

    static adapter_type create(IteratorType first, IteratorType last)
    {
        return adapter_type(
            base_adapter_type(std::move(first), std::move(last)));
    }
};
...
{
    return container_input_adapter_factory_impl::
        container_input_adapter_factory<ContainerType>::create(container);
}
